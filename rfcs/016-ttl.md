...
4. Optional read-path TTL filtering, controlled by
   ...
   on point reads (`get`, `batch_get`) even before compaction rewrites the
   containing SST. Scan operations intentionally do NOT apply TTL filtering —
   per-key `expire_at` checks on every scanned key would be prohibitively
   expensive; TTL cleanup for scanned data is handled at compaction time.
...
6. TTL compaction priority integrated into the leveled compaction controller:
   ...
   selected for compaction to reclaim space without waiting for natural
   compaction to visit them.
...
5. Provide optional read-path TTL filtering (opt-in via
   ...
   point reads (`get`, `batch_get`) even before compaction removes them. Scan
   operations intentionally do not filter — per-key checks are too expensive;
   cleanup is compaction-time.
...
Layer 2: Visibility (opt-in read-path filtering, point reads only)
  get / batch_get check: if kind is TTL and now_secs > expire_at, return None
  ...
  Scan operations do NOT filter — per-key checks on every scanned key would be
  prohibitively expensive; TTL cleanup for scan-visible data is compaction-time.
...
Layer 3: Cleanup (compaction-time)
  ...
  Three-tier cleanup:
    ...
    b) TTL compaction priority — the leveled controller selects mixed SSTs
       with TTL ratio ≥ 0.5 for priority compaction.
...
When `ttl_read_filtering` is enabled, point-read operations (`get`, `batch_get`)
apply an additional visibility check after the MVCC version has been selected.
A `wall_clock_secs()` helper pre-fetches the current time once per top-level
request to avoid repeated `SystemTime::now()` syscalls:
...
if candidate is None: return None
  ↓
if ttl_read_filtering and is_ttl_expired(now_secs, candidate.expire_at): return None
  ↓
return candidate.value
```

**Scan operations (`scan`, `prefix_scan`) do NOT apply TTL filtering.** Per-key
`expire_at` checks on every scanned key would be prohibitively expensive,
especially for large range scans. TTL cleanup for scanned data relies on
compaction-time mechanisms (wholesale drop, per-entry `should_drop_for_ttl`,
and TTL compaction priority). This is an intentional design tradeoff: point
reads get immediate filtering (O(1) check), scans get eventual cleanup via
compaction. Callers who need scan-level TTL filtering should combine point-read
TTL filtering with compaction-time cleanup.
...
The SST with the highest TTL count is selected for priority compaction.
This ensures space reclamation for expired entries without unnecessary rewrites
of SSTs with few expired entries. The compaction pipeline handles per-entry
`should_drop_for_ttl` to remove individual expired entries while preserving
non-expired ones.
...
pub struct LsmStorageOptions {
    // ... existing fields ...
    /// Enable read-path TTL filtering on point reads. Default: false.
    pub ttl_read_filtering: bool,
}
```
...
6. With `ttl_read_filtering = true`, expired key returns `None` even before
   compaction.
7. With `ttl_read_filtering = true`, a point read returns `None` for expired
   keys. Scan operations do not filter — they return all keys regardless of
   TTL expiration.
...
| `scan` (filtering N/A) | 1× | 1× (no filtering) | 1× (no filtering) |
...
- **`SystemTime::now()` cost**: When `ttl_read_filtering` is enabled, each
  point-read operation calls `wall_clock_secs()` once per top-level request
  (not per-key). On Linux, this is ~20–25ns via vDSO (`clock_gettime`).
  Scan operations do not call `SystemTime::now()` at all — they bypass TTL
  filtering entirely.
...
1. Point-get throughput with `ttl_read_filtering` enabled vs disabled: no more
   than 5% regression.
2. Scan throughput: no change (scan path does not apply TTL filtering).
...
4. Implement `try_ttl_wholesale_drop` with all safety checks: fully-expired
   ...
   succeeds. Filter empty levels when computing effective bottom level.
5. Integrate TTL compaction priority into leveled controller via
   ...
   for priority compaction. Bottom-level mixed SSTs self-compact (reads SST,
   drops expired per-entry, writes back).
...
### Phase 4: Read-Path Filtering (Optional/Opt-in, Point Reads Only)

1. Add `LsmStorageOptions::ttl_read_filtering`.
2. Implement TTL expiry check in the `get` and `batch_get` paths, after MVCC
   version selection. Pre-fetch `now_secs` once per top-level request using
   the `wall_clock_secs()` helper.
3. Scan and prefix_scan intentionally do NOT apply TTL filtering — per-key
   checks on every scanned key are prohibitively expensive. Cleanup is via
   compaction.
4. Write integration tests for read-path point-read filtering.
...
7. Document TTL semantics: point-read filtering only, scan cleanup via compaction,
   wall-clock caveats.
