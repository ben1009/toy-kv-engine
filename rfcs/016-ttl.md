...
4. Read-path TTL visibility that returns `None` for expired keys on point reads
   (`get`, `batch_get`) and scan operations (`scan`, `prefix_scan`) even before
   compaction rewrites the containing SST. Scan iterators capture `now_secs`
   once at iterator creation so the full cursor sees a stable TTL view without
   per-row wall-clock syscalls.
5. Per-SST TTL metadata (`min_ttl_expire_ts`, `max_ttl_expire_ts`,
   `ttl_entry_count`, `total_entry_count`, `has_non_ttl_entries`) for O(1)
   compaction-level TTL eligibility checks, allowing entire SSTs to be skipped or
   wholesale-dropped, and TTL ratio-gated compaction priority.
6. TTL compaction priority integrated into the leveled compaction controller:
   mixed SSTs with a high proportion of expired TTL entries (ratio ≥ 0.5) are
   selected for bottom-level compaction to reclaim space without waiting for
   natural compaction to visit them. Physical removal still stays bottom-level
   only to avoid resurrecting older versions from lower levels.
...
5. Provide read-path TTL filtering so that expired keys are invisible to point
   reads (`get`, `batch_get`) and scans (`scan`, `prefix_scan`) even before
   compaction removes them. Scan iterators capture one `now_secs` snapshot at
   construction time so TTL checks stay stable for the life of the cursor.
...
5. **TTL as a first-class MVCC isolation boundary.** The MVP does not add
   wall-clock time to `ReadGuard`. Point reads capture `now_secs` per request
   and scans capture it per iterator, but separate requests in the same logical
   application workflow can still observe TTL transitions as wall clock time
   advances.
...
Layer 2: Visibility (read-path filtering)
  get / batch_get / scan / prefix_scan check:
    if kind is TTL and now_secs >= expire_at, return None
  `now_secs` is pre-fetched once per top-level request (wall_clock_secs() helper)
  to avoid repeated SystemTime::now() syscalls on hot paths.
  Scan iterators capture `now_secs` once at iterator creation and reuse it for
  every row, so scans get stable TTL semantics without per-row clock reads.

Layer 3: Cleanup (compaction-time)
  During compaction, drop TTL entries where now_secs >= expire_at
  Three-tier cleanup:
    a) Wholesale drop — fully-expired pure-TTL SSTs at bottommost level
       are deleted without reading any data blocks.
    b) TTL compaction priority — the leveled controller selects bottom-level
       mixed SSTs with TTL ratio ≥ 0.5 for priority compaction.
    c) Per-entry drop — during normal compaction, should_drop_for_ttl removes
       individual expired entries.
...
impl KvEngine {
    ...
    ///
    /// The key becomes invisible to reads once `now() > expire_at`.
    ///
...
#### 6.4.1 Always-On TTL Visibility Filtering

The read path always enforces TTL visibility using the stored `expire_at`
timestamp:
...
Point-read operations (`get`, `batch_get`) and scan operations (`scan`,
`prefix_scan`) apply an additional visibility check after the MVCC version has
been selected. A `wall_clock_secs()` helper
pre-fetches the current time once per top-level request or once per scan
iterator to avoid repeated `SystemTime::now()` syscalls:
...
fn is_ttl_expired(now_secs: u64, expire_at_secs: u64) -> bool {
...
```text
now_secs = wall_clock_secs()  // pre-fetched once
candidate = newest visible point version at read_ts
  ↓
if candidate is None: return None
  ↓
if is_ttl_expired(now_secs, candidate.expire_at): return None
  ↓
return candidate.value
```

**Scan operations (`scan`, `prefix_scan`) also apply TTL filtering.** Each scan
captures `now_secs` once at iterator construction and reuses that timestamp for
the full cursor. This gives the iterator a stable TTL view while avoiding
per-row `SystemTime::now()` syscalls. Expired TTL entries behave like
tombstones in the scan path: they suppress older versions of the same user key
instead of falling through to an earlier value.

#### 6.4.2 Interaction With Range Tombstones
...
pub struct SsTableTtlMetadata {
    ...
    /// Tombstone, range tombstones). If true, the SST cannot be
    /// wholesale-dropped even if all TTL entries have expired, because
    /// non-TTL entries never expire.
    /// Range tombstones force this to true — tombstones never expire and
    /// wholesale-dropping an SST with range tombstones would resurrect
    /// older versions in lower levels.
    pub has_non_ttl_entries: bool,
    /// Number of TTL entries in this SST. Used for TTL ratio calculation
    /// in compaction priority (ttl_entry_count / total_entry_count).
    pub ttl_entry_count: u64,
    /// Total number of entries in this SST (TTL + non-TTL).
    pub total_entry_count: u64,
}
```

The `ttl_entry_count` and `total_entry_count` fields enable the compaction
controller to compute the TTL ratio (`ttl_entry_count / total_entry_count`) and
prioritize compaction of SSTs where ≥50% of entries are expired. This replaces
the planned background scanner with an integrated, zero-overhead approach.
Malformed TTL entries (<9 bytes) are treated as non-TTL by the SST builder to
prevent inflating the TTL ratio.
...
**Optimization 2: Wholesale-drop SSTs with all entries expired.** Implemented
as `try_ttl_wholesale_drop()` in `compact.rs`. The method is called at the end
of each `trigger_compaction` cycle and performs metadata-only deletion of
fully-expired pure-TTL SSTs:

1. **Candidate scan (no lock):** Collect SST IDs where
   `max_ttl_expire_ts > 0 && max_ttl_expire_ts <= now_secs`.
2. **Under `state_lock`:** Determine the effective bottom level by filtering
   out empty levels, then keep only candidates at that level.
3. **State mutation:** Remove SSTs from the snapshot, persist a new manifest
   snapshot, store the new state, then unregister vLog references and delete
   SST files from disk. vLog cleanup is deferred to after `state.store` to
   avoid leaking refs on manifest failure.
4. **Block cache invalidation:** Remove cached blocks belonging to deleted SSTs.

The safety conditions are implicitly satisfied: only bottommost-level pure-TTL
SSTs pass (`has_non_ttl_entries = false` ensures no range tombstones, since
range tombstones force `has_non_ttl_entries = true`); the MVCC watermark gate
is checked before the state mutation; and `_state_lock` prevents concurrent
state modifications. Since all compaction runs synchronously under
`trigger_compaction`, there is no race with active compaction tasks.
...
pub struct SsTableBuilder {
    ...
    /// Number of TTL entries added.
    ttl_entry_count: u64,
    /// Total number of entries added (TTL + non-TTL).
    total_entry_count: u64,
    /// Whether any non-TTL entries were added. Range tombstones
    /// force this to true.
    has_non_ttl_entries: bool,
}
```

When `add_raw` or `add_with_ttl` is called with a TTL-prefixed value, the
builder extracts `expire_at_secs` and updates the metadata counters.
`add_with_ttl` routes through `add_raw` for TTL value pointers so that
`referenced_vlog_ids` tracking is correctly applied. Malformed TTL entries
(<9 bytes) are treated as non-TTL to prevent inflating the TTL ratio.
...
#### 6.6.1 Footer Version 9

A new footer version extends v8 with TTL entry count fields:

```text
SST_FOOTER_VERSION_V9: u8 = 9
```

Footer v9 layout (58 bytes):
...
------  ----  -----
 ...
 12      8    min_ttl_expire_ts: u64
 20      8    max_ttl_expire_ts: u64
 28      1    has_non_ttl_entries: u8        (0 or 1; range tombstones force 1)
 29      8    max_ts: u64
 37      8    ttl_entry_count: u64
 45      8    total_entry_count: u64
 53      4    magic: u32 (0x4D56_4343)
 57      1    version: u8 = 9
```
...
v9 builds on v8 (42 bytes: min_ttl/max_ttl/has_non_ttl/max_ts) by adding
`ttl_entry_count` and `total_entry_count`. These enable O(1) TTL ratio
calculation for compaction priority without scanning data blocks.

V9 is an MVCC format version and uses stable bloom hashing, same as v5–v8.
The `is_mvcc` and `stable_bloom_hash` checks in `SsTable::open` must include V9.

Legacy SSTs (v2–v8) implicitly have `min_ttl_expire_ts = u64::MAX`,
`max_ttl_expire_ts = 0`, `ttl_entry_count = 0`, and `total_entry_count` equal
to their actual entry count. `SsTable::open` sets these defaults for non-v9 SSTs.

#### 6.6.2 Backward Compatibility

| Artifact | Old files read by v9 binary | New v9 artifact read by old binary |
|---|---|---|
| Manifest v4 | Yes | N/A (manifest handles this) |
| SST v2–v8 | Yes (TTL metadata defaults applied) | N/A |
| SST v9 | N/A | No (rejected by version check) |
| WAL | Yes (TTL entries are opaque bytes) | N/A |

The value encoding is structured so that TTL entries appear as opaque bytes to
older readers: a v8 binary would see the raw `[0x03][expire_at: u64 BE][value...]`
sequence as the value. However, older binaries still reject v9 SSTs and manifest
v5 through version checks — there is no safe downgrade path once the manifest
and SSTs have been upgraded. Downgrade is unsupported.
...
binaries reject databases containing SST v9 files (which include TTL metadata in
their footers).
...
v9 files, but the manifest does not need to know the SST format version; that is
determined when `SsTable::open` reads the footer.

**Upgrade behavior:**

1. Existing manifest v4 databases open under a v5 binary.
2. A manifest v5 snapshot is written atomically before any SST v9 is created.
3. Existing SSTs (v2–v8) remain readable; new SSTs are written as v9.
...
insert. SST recovery already reads `max_ts` from the footer; v9 SSTs additionally
read `min_ttl_expire_ts`, `max_ttl_expire_ts`, `has_non_ttl_entries`,
`ttl_entry_count`, and `total_entry_count`.
...
be left in the SST, and subsequent reads could observe reclaimed vLog data.
Expiration is handled by compaction plus the always-on TTL visibility check on
the read path.
...
### 6.10 TTL Compaction Priority

Instead of a standalone background scanner, TTL compaction is integrated directly
into the leveled compaction controller. This avoids the complexity of a separate
background task, eliminates coordination challenges with active compactions, and
reuses the existing compaction pipeline.

#### 6.10.1 Wholesale Drop of Fully-Expired Pure-TTL SSTs

`try_ttl_wholesale_drop` is called at the end of each `trigger_compaction` cycle.
It identifies SSTs at the bottommost non-empty level where all entries have
expired:

1. Scan all SST IDs for `max_ttl_expire_ts > 0 && max_ttl_expire_ts <= now_secs`.
2. Under `state_lock`, determine the effective bottom level by filtering out
   empty levels (a configured max level like L4 may have no SSTs while L3 is the
   actual bottommost non-empty level).
3. Filter candidates to only bottom-level SSTs (other levels cannot be
   wholesale-dropped — their entries shadow older versions in lower levels).
4. Remove SSTs from the in-memory snapshot, persist a new manifest snapshot,
   store the new state, then unregister vLog references and delete SST files
   from disk. vLog references are cleaned up **after** `state.store` succeeds to
   avoid leaking refs if manifest persistence fails.
   While publishing this transition, hold the MVCC reader-registration barrier
   so no new `ReadGuard` can slip in between the no-active-readers safety check
   and the physical file deletion.
5. Invalidate cached blocks from deleted SSTs.

#### 6.10.2 TTL Compaction Priority in Leveled Controller

The leveled compaction controller's `generate_ttl_compaction_task` identifies
mixed SSTs (both TTL and non-TTL entries) with a high proportion of expired
entries. It scans all levels for SSTs where:

- `max_ttl_expire_ts > 0` (has TTL entries)
- `max_ttl_expire_ts <= now_secs` (at least some entries are expired)
- `ttl_entry_count / total_entry_count >= 0.5` (≥50% TTL entries)

The SST with the highest TTL ratio is selected for priority compaction among
bottom-level candidates. Even though expired TTL entries are logically hidden on
the read path, physically deleting an expired version above the bottom could
still expose an older version from a lower level. Bottom-level-only reclamation
avoids that resurrection hazard.

The compaction pipeline handles per-entry `should_drop_for_ttl` to remove
individual expired entries while preserving non-expired ones.

For bottom-level mixed SSTs, the task is generated as a self-compaction
(upper_level == lower_level). This reads the SST, drops expired entries via
`should_drop_for_ttl`, and writes the surviving entries back to the same level.

> **Design rationale.** The background scanner was removed because:
> - It required complex coordination with active compactions.
> - The integrated approach reuses existing compaction safety guarantees
>   (MVCC watermark, state lock, manifest snapshots).
> - Wholesale drop provides the same O(SSTs) metadata-only deletion for
>   pure-TTL SSTs.
> - TTL compaction priority replaces the scanner's prioritization role
>   for mixed SSTs.
> - The `ttl_background_scanner_interval` option was removed.
...
pub struct TtlStats {
    ...
    /// Number of read yields suppressed by TTL visibility checks.
...
8. SST v9 footer encode/decode round-trips TTL metadata, including sentinel
   values for SSTs with no TTL entries.
9. SST v8 opened by v9 binary has default TTL metadata
   (`min = u64::MAX, max = 0, ttl_entry_count = 0`).
...
5. WriteBatch with mixed `Put`, `PutWithTtl`, `Del`: all three operations apply
   correctly.
6. Expired key returns `None` even before
   compaction.
7. Point reads and scans both hide expired keys.
...
14. Manifest upgrade: v4 database opens under v5 binary, manifest snapshot is
    upgraded before first SST v9 is written.
15. Downgrade rejection: older binary rejects manifest v5 and SST v9.
16. SST v9 with range tombstones and TTL metadata: footer contains both
    `range_tombstone_offset` and TTL fields, and `has_non_ttl_entries = true`
    (range tombstones never expire).
17. TTL compaction priority correctly identifies mixed SSTs with high TTL ratio
    for priority compaction.
...
5. SST open/read tests for v2–v8 files.
...
| `get` | 1× | ~1× + 1 `SystemTime::now()` call | ~1× + 1 call |
| `scan` | 1× | ~1× + 1 iterator-scoped `SystemTime::now()` call | ~1× + 1 call |
...
- **SST footer overhead**: 16 bytes per SST for TTL metadata (min_ttl + max_ttl
  + has_non_ttl + ttl_entry_count + total_entry_count vs sentinels). An SST of
  64MB with footer overhead of 58 bytes (v9) vs 42 bytes (v8) = ~0.00002%.
...
- **`SystemTime::now()` cost**: Each point-read operation calls
  `wall_clock_secs()` once per top-level request (not per-key). Each scan calls
  it once per iterator (not per-row). On Linux, this is ~20–25ns via vDSO
  (`clock_gettime`).
...
1. Point-get throughput with TTL visibility enabled: no more than 5%
   regression.
2. Scan throughput with iterator-scoped TTL visibility: no more than 5%
   regression.
...
### Phase 2: SST Format v9 and Metadata

1. Define `SST_FOOTER_VERSION_V9` and the 58-byte footer layout.
2. Extend `SsTableBuilder` to track TTL metadata: `min_ttl_expire_ts`,
   `max_ttl_expire_ts`, `ttl_entry_count`, `total_entry_count`, and
   `has_non_ttl_entries`. Range tombstones force `has_non_ttl_entries = true`.
   Malformed TTL entries (<9 bytes) are treated as non-TTL.
3. Extend `SsTable::open` to read v9 footer and populate `ttl_metadata`.
   Include V9 in `is_mvcc` and `stable_bloom_hash` checks.
4. Update `SsTable` struct with `ttl_metadata: SsTableTtlMetadata`.
5. Add backward-compatible defaults for v2–v8 SSTs.
6. Write integration tests for SST v9 encode/decode with TTL metadata.
...
3. Implement per-SST TTL fast-path: skip TTL checks when SST has no TTL entries
   (`max_ttl_expire_ts == 0`).
4. Implement `try_ttl_wholesale_drop` with all safety checks: fully-expired
   pure-TTL SSTs, bottommost non-empty level, under `can_publish_ttl_deletion`,
   no range tombstones (`has_non_ttl_entries` guards this), no installed
   compaction filters. Defer vLog reference cleanup to after `state.store`
   succeeds. Filter empty levels when computing effective bottom level. Hold the
   MVCC reader-registration barrier across the publish/delete sequence so no new
   reader can start on the old snapshot during file reclamation.
5. Integrate TTL compaction priority into leveled controller via
   `generate_ttl_compaction_task`: select mixed SSTs with
   `max_ttl_expire_ts <= now_secs && ttl_entry_count/total_entry_count >= 0.5`
   for bottom-level priority compaction.
6. Update `check_liveness` and `compact_file` in `vlog/gc.rs` to handle
   `TtlValuePointer` (decode pointer from offset 9, use pre-parsed `expire_at`
   from tuple to avoid re-parsing, preserve TTL prefix on rewrite with
   `Liveness::Live { expire_at }`).
7. Add `wall_clock_secs()` helper to avoid repeated `SystemTime::now()` syscalls.
   Thread `now_secs: u64` through `resolve_value` and `is_ttl_expired`.
8. Add `current_sst_id()` non-advancing accessor for snapshot path (vs
   `next_sst_id()` which increments).
9. Block cache invalidation after wholesale SST removal.
10. Write integration tests for TTL compaction, wholesale drop, read guard gate,
    and vLog GC with TTL entries.

### Phase 4: Read-Path Visibility

1. Implement TTL expiry check in the `get` and `batch_get` paths, after MVCC
   version selection. Pre-fetch `now_secs` once per top-level request using
   the `wall_clock_secs()` helper.
2. Implement TTL expiry checks in `scan` and `prefix_scan` by capturing one
   `now_secs` value per iterator and reusing it for the full cursor.
3. Write integration tests for point-read and scan TTL filtering.

### Phase 5: Public API, Manifest, and Final Integration

1. Expose `KvEngine::put_with_ttl`.
2. Bump `MANIFEST_FORMAT_VERSION` to 5.
3. Implement manifest v4-to-v5 upgrade (write v5 snapshot before first SST v9).
4. Add `WriteBatchRecord::PutWithTtl` with `u64` TTL seconds and
   `BatchEntryKind` enum (`Put`, `Delete`, `PutTtl`) to replace byte-sniffing.
5. Add TTL stats: `ttl_puts`, `ttl_entries_dropped`, `ttl_bytes_dropped`,
   `ttl_ssts_wholesale_dropped`, `ttl_read_suppressions`.
6. Add benchmark coverage and enforce performance gates (zero regression).
7. Document TTL semantics: always-on point-read and scan filtering,
   bottom-level-only physical reclamation, and wall-clock caveats.
...
3. **Per-scan TTL override.** Should scans optionally ignore TTL visibility for
   diagnostic or administrative tooling?
...
4. **TTL compaction priority.** ~~Boost compaction priority for levels containing
   SSTs with a high fraction of expired TTL entries.~~ **Implemented:** The
   leveled controller's `generate_ttl_compaction_task` already targets mixed
   SSTs with TTL ratio >= 0.5 for priority compaction.
